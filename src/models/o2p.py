...
class ConvertedModel(nn.Module):
    ...
    def __init__(self, onnx_model, verbose=False, model_name='scanoma', pretrained=True):
        ...
        for tensor in graph.initializer:
            data = torch.from_numpy(onnx.numpy_helper.to_array(tensor).copy())
            ...
            if not pretrained and model_name == 'sscd':
                if 'final_weights/transpose' in tensor.name:
                    weights = nn.Parameter(torch.empty((2, 1001), requires_grad=requires_grad))
                    init.kaiming_uniform_(weights, a=math.sqrt(5))
                    gemm_weights = weights
                    self.parameter_dict[tensor.name.replace('.','')] = gemm_weights
                elif 'final_training_ops/Wx_plus_b/MatMul_bias' in tensor.name:
                    bias = nn.Parameter(torch.empty((2), requires_grad=requires_grad))
                    fan_in, _ = init._calculate_fan_in_and_fan_out(gemm_weights)
                    bound = 1 / math.sqrt(fan_in) if fan_in > 0 else 0
                    init.uniform_(bias, -bound, bound)
                    self.parameter_dict[tensor.name.replace('.','')] = bias
                else:
